...
/**
 @author Tetra.win
 @title UsingSignum
 @dev This contract helps smart contracts read data from Signum
 */
contract UsingSignum {
   ...
   */
  function _getDataAfter(
    bytes32 _queryId,
    uint256 _timestamp
  ) internal view returns (bytes memory _value, uint256 _timestampRetrieved) {
    (bool _found, uint256 _index) = _getIndexForDataAfter(_queryId, _timestamp);
    if (!_found) {
      return ("", 0);
    }
  ...
  function _getIndexForDataAfter(
    bytes32 _queryId,
    uint256 _timestamp
    ...
    while (_search) {
      ...
      _timestampRetrieved = _getTimestampbyQueryIdandIndex(_queryId, _middle);
      if (_timestampRetrieved > _timestamp) {
    ...
    if (!_isInDispute(_queryId, _timestampRetrieved)) {
      ...
      while (_isInDispute(_queryId, _timestampRetrieved) && _middle < _count) {
        _middle++;
        _timestampRetrieved = _getTimestampbyQueryIdandIndex(_queryId, _middle);
      }
      if (_middle == _count && _isInDispute(_queryId, _timestampRetrieved)) {
        return (false, 0);
      }
      // _timestampRetrieved is correct
      return (true, _middle);
   ...
   */
  // slither-disable-next-line calls-loop
  function _getIndexForDataBefore(
    bytes32 _queryId,
    uint256 _timestamp
  ) internal view returns (bool _found, uint256 _index) {
   ...
   */
  function _getNewValueCountbyQueryId(
    bytes32 _queryId
   ...
   */
  function _getReporterByTimestamp(
    bytes32 _queryId,
    uint256 _timestamp
   ...
   */
  function _getTimestampbyQueryIdandIndex(
    bytes32 _queryId,
    uint256 _index
  ) internal view returns (uint256) {

...
describe("UsingSignum Function Tests", function() {
  ...
  const TRB_QUERY_ID = ethers.utils.keccak256(TRB_QUERY_DATA);
	...
	beforeEach(async function () {
		...
		const SignumPlayground = await ethers.getContractFactory("SignumPlayground");
    token = await SignumPlayground.deploy();
    await token.deployed();

    const SignumFlex = await ethers.getContractFactory("SignumFlex");
    oracle = await SignumFlex.deploy(token.address, 86400/2, h.toWei("15"), h.toWei("1500"), h.toWei(".001"), TRB_QUERY_ID);
    ...
    const BenchUsingSignum = await ethers.getContractFactory("BenchUsingSignum");
    bench = await BenchUsingSignum.deploy(oracle.address);
